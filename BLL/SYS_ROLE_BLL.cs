...
namespace BLL  
{
	...
	{	
        ...
        /// <returns></returns>
        public bool Add(SYS_ROLE model)
        {
            bool success = false;
            using (var dbcontext = DbFactory.Create())
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Logger.Error(string.Format("SYS_ROLE_BLL 新增记录异常,异常信息:{0}", ex.ToString()));
                }
            }
        ...
        /// <returns></returns>
		public bool Remove(SYS_ROLE model)
        {
            bool success = false;
            ...
            using (var dbcontext = DbFactory.Create())
            {
                ...
                {
                    success = false;
                    Logger.Error(string.Format("SYS_ROLE_BLL 删除记录异常,异常信息:{0}", ex.ToString()));
                }
            }
        ...
        public bool Remove(string pkid)
        {
            ...
            using (var dbcontext = DbFactory.Create())
            {
                ...
                {
                    success = false;
                    Logger.Error(string.Format("SYS_ROLE_BLL 根据id删除异常,异常信息:{0}", ex.ToString()));
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public bool BulkRemove(Expression<Func<SYS_ROLE, bool>> exp)
        {
            bool success = false;
            ...
            using (var dbcontext = DbFactory.Create())
            {
                ...
                {
                    success = false;
                    Logger.Error(string.Format("SYS_ROLE_BLL 根据条件批量删除,异常信息:{0}", ex.ToString()));
                }
            }
        ...
        /// <returns></returns>
        public bool Update(Expression<Func<SYS_ROLE, bool>> exp, Dictionary<string, object> dic)
        {
            bool success = false;
           	...
           	using (var dbcontext = DbFactory.Create())
            {
                ...
                {
                    success = false;
                    Logger.Error(string.Format("SYS_ROLE_BLL 按条件更新,异常信息:{0}", ex.ToString()));
                }
            }
